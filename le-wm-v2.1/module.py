...
class Mamba3ConditionalBlock(nn.Module):
    """Transformer block với AdaLN-zero — thay FeedForward bằng Mamba-3"""

    def __init__(self, dim, heads, dim_head, d_state=128, headdim=64, dropout=0.0):
        super().__init__()
        from mamba_ssm import Mamba3

        self.attn = Attention(dim, heads=heads, dim_head=dim_head, dropout=dropout)
        self.mamba3 = Mamba3(
            d_model=dim, d_state=d_state, headdim=headdim, is_mimo=False,
        )
        self.norm1 = nn.LayerNorm(dim, elementwise_affine=False, eps=1e-6)
        self.norm2 = nn.LayerNorm(dim, elementwise_affine=False, eps=1e-6)
        self.adaLN_modulation = nn.Sequential(
    ...
    def forward(self, x, c):
        """
        x: (B, T, D)
        c: (B, T, D) — action embedding
        """
        shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = (
            self.adaLN_modulation(c).chunk(6, dim=-1)
        )
        # Attention (spatial mixing) — giữ nguyên
        x = x + gate_msa * self.attn(modulate(self.norm1(x), shift_msa, scale_msa))
        # Mamba-3 (temporal + per-step) — thay MLP
        x = x + gate_mlp * self.mamba3(modulate(self.norm2(x), shift_mlp, scale_mlp))
        return x


class Mamba3Transformer(nn.Module):
    """Transformer dùng Mamba3ConditionalBlock"""

    def __init__(
        self, input_dim, hidden_dim, output_dim, depth, heads, dim_head,
        d_state=128, headdim=64, dropout=0.0,
    ):
        super().__init__()
        self.norm = nn.LayerNorm(hidden_dim)
        self.layers = nn.ModuleList([])
        ...
        for _ in range(depth):
            self.layers.append(
                Mamba3ConditionalBlock(hidden_dim, heads, dim_head, d_state, headdim, dropout)
            )
...
class Mamba3Predictor(nn.Module):
    """V2: Hybrid Attention + Mamba-3 predictor.
    Giống ARPredictor nhưng thay ConditionalBlock (Attn→MLP) bằng Mamba3ConditionalBlock (Attn→Mamba-3).
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.transformer = Mamba3Transformer(
            input_dim, hidden_dim, output_dim or input_dim,
            depth, heads, dim_head, d_state, headdim, dropout,
        )
...
class Mamba2ConditionalBlock(nn.Module):
    ...
    def forward(self, x, c):
        ...
        x = x + gate_msa * self.attn(modulate(self.norm1(x), shift_msa, scale_msa))
        x = x + gate_mlp * self.mamba2(modulate(self.norm2(x), shift_mlp, scale_mlp))
        return x
...
class Mamba2Transformer(nn.Module):
    ...
    def forward(self, x, c=None):
        ...
        for block in self.layers: x = block(x, c)
        return self.norm(self.output_proj(x))


class Mamba2Predictor(nn.Module):
    """V2.1: Attention + Mamba-2 hybrid predictor.
    Giống ARPredictor nhưng thay MLP bằng Mamba-2.
    Dùng Mamba-2 vì có wheel cho CUDA 13, ko cần build từ source.
    """
